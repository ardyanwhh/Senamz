import React from 'react';
import { View, Text, SafeAreaView, Image } from 'react-native';
import usStyle from '../assets/css/usStyle';

export default Ana = () => (
  <SafeAreaView style={usStyle.container}>
    <View style={usStyle.profilePicContainer}>
      <Image style={usStyle.profilePic} source={require('../assets/img/Ana.jpg')}/>
    </View>
    <View style={usStyle.nameTextContainer}>
      <Text style={usStyle.nameText}>Ana Yuni Zafira</Text>
    </View>
    <View>
      <Text>{`UIUX Designer`}</Text>
    </View>
    <View style={usStyle.emailTextContainer}>
      <Text style={usStyle.emailText}>anayuniz.202@gmail.com</Text>
    </View>
  </SafeAreaView>
);